...
export type PoseTrackerStatus = 'idle' | 'configuring' | 'downloading' | 'warming' | 'ready' | 'error';
/**
...
export interface ErrorEvent {
    type: 'error';
    code: 'invalid_token' | 'quota_exceeded' | 'network'
    /**
    ...
    /** jump_analysis requires `userHeightCm` (WebView parity). */
     | 'jump_analysis_missing_height' | 'model_load_failed' | 'engine_load_failed' | 'integrity_check_failed' | 'webview_error' | 'device_too_slow' | 'backend_fallback_wasm' | 'internal';
    message: string;
}
/**
...
export interface WarningEvent {
    type: 'warning';
    code: 'quality_downgraded' | 'backend_fallback_wasm' | 'webview' | 'internal';
    message: string;
    timestampMs: number;
}
...
export interface QualityChangedEvent {
    ...
    reason: 'low_fps' | 'device_capability' | 'crash_guard' | 'warmup_estimate' | 'manual';
    detail: string;
...
export type PoseTrackerEvent = InitializationEvent | ErrorEvent | WarningEvent | RuntimeDownloadProgressEvent | KeypointsEvent | AnglesEvent | CounterEvent | PostureEvent | ProgressionEvent | EngineDebugEvent | RecommendationsEvent | FormScoreEvent | ExerciseSummaryEvent | JumpCalibrationEvent | JumpStartedEvent | JumpHeightEvent | JumpDiscardedEvent | JumpResultEvent | JumpSummaryEvent | QualityChangedEvent | PerformanceWarningEvent;
