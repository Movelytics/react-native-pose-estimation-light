...
import { ENABLE_MALI_HARD_CAP } from './captureMode';
...
function probeTotalMemoryGiB(): number | null {
  // Optional peers — never required.
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const Device = require('expo-device');
    const bytes = Device.totalMemory as number | null | undefined;
    if (typeof bytes === 'number' && bytes > 0) return bytes / (1024 * 1024 * 1024);
  } catch {
    /* ignore */
  }
  try {
    ...
    if (typeof DeviceInfo.getTotalMemorySync === 'function') {
      const bytes = DeviceInfo.getTotalMemorySync() as number;
      if (typeof bytes === 'number' && bytes > 0) return bytes / (1024 * 1024 * 1024);
    }
  } catch {
    /* ignore */
...
function parseOsVersion(): number | null {
...
export function isMaliRenderer(renderer: string | null | undefined): boolean {
 ...
 */
export function profileFromScore(score: number): QualityProfileId {
  if (score >= 78) return 'prime';
  if (score >= 62) return 'pro';
  if (score >= 48) return 'lite';
  if (score >= 32) return 'ultralite';
  return 'basic';
}

export function scoreDeviceCapability(options?: {
  glRenderer?: string | null;
