...
export function deriveCacheSecret(apiToken: string): string {
  return sha256(`posetracker-sdk-cache:${apiToken}`);
}
...
function xorWithKeystream(bytes: Uint8Array, secret: string): Uint8Array {
  const out = new Uint8Array(bytes.length);
  let counter = 0;
  let offset = 0;
  while (offset < bytes.length) {
    const block = sha256.array(`${secret}:${counter++}`);
...
export function sealString(plain: string, secret: string): string {
...
export function openString(sealed: string, secret: string): string | null {
